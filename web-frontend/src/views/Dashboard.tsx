...
type DashboardChart = {
    data?: (elm: HTMLCanvasElement) => ChartData
    options: ChartOptions
}

interface Charts {
    [key: string]: DashboardChart | null
}

type MonthlyTransactionStatistics = {
    transactions: number
    sum: number
    balance: number
}
...
class Dashboard extends React.Component<DashboardProps, DashboardState> {
    ...
    transactionsChartData(label: string, results: any[]): (canvas: HTMLCanvasElement) => ChartData {
    ...
    componentDidMount() {
        fetch("http://api.fima.test/transaction/statistics") // FIXME: Don't use hardcoded domain name
            .then(res => res.json())
            .then((results: MonthlyTransactionStatistics[]) => {
                this.setState({
                        charts: {
                            count: {
                            ...
                            <i className="tim-icons icon-tap-02" />
                                ...
                                <CardBody>
                                    <div className="chart-area">
                                        <ChartComponent
                                            data={this.state.charts[this.state.selectedChart]?.data}
                                            options={this.state.charts[this.state.selectedChart]?.options}
                                            type='line' />
                                    </div>
                                </CardBody>
                            </Card>
